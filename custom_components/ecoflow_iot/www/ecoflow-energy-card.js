...
`;function vt(r){let t=[];for(let e=1;e<=4;e++){let s=r._config.panels?.[e]||{};if(s.hidden)continue;let o=`sensor.pv${e}_power`,i=r._state(o);i&&t.push({i:e,watts:b(i),id:r._entityId(o),name:s.name||null})}return t}function Qt(r){let t=(i,n)=>S(r.hass,i,n),e=vt(r);if(!e.length)return c`<div class="empty">${t("panels.none")}</div>`;let s=Math.max(1,...e.map(i=>i.watts||0)),o=e.reduce((i,n)=>i+(n.watts||0),0);return c`<div class="panels">
    ${e.map(i=>c`<div
  ...
  </div>`}var yt=1e3,bt=340,v={l:46,r:14,t:16,b:28};function te(r,{actual:t,forecast:e,totalWh:s,showForecast:o,title:i}){let n=(p,m)=>S(r.hass,p,m),l=yt-v.l-v.r,a=bt-v.t-v.b,d=p=>v.l+p/24*l,u=0;for(let p=0;p<24;p++)u=Math.max(u,(t[p]||0)/1e3),o&&(u=Math.max(u,(e[p]||0)/1e3));let h=Ae(u>0?u:.1),_=p=>v.t+a-p/h*a,y=l/24*.66,E=[];for(let p=0;p<24;p++){let m=(t[p]||0)/1e3;if(m<=0)continue;let D=d(p+.5)-y/2,kt=_(m);E.push(z`<rect class="fc-actual" x=${D.toFixed(1)} y=${kt.toFixed(1)}
        width=${y.toFixed(1)} height=${(v.t+a-kt).toFixed(1)} rx="2"></rect>`)}let $t=null;if(o){let p=[];for(let m=0;m<=24;m++)p.push(`${d(m).toFixed(1)},${_((e[m]||0)/1e3).toFixed(1)}`);$t=z`<polyline class="fc-line" points=${p.join(" ")}></polyline>`}let et=[],xt=4;for(let p=0;p<=xt;p++){let m=h/xt*p,D=_(m);et.push(z`<line class="fc-grid" x1=${v.l} y1=${D.toFixed(1)} x2=${yt-v.r} y2=${D.toFixed(1)}></line>`),et.push(z`<text class="fc-axis fc-axis-y" x=${v.l-6} y=${(D+4).toFixed(1)}>${Se(m)}</text>`)}let wt=[];for(let p=0;p<=24;p+=4)wt.push(z`<text class="fc-axis fc-axis-x" x=${d(p).toFixed(1)} y=${bt-8}>${p}:00</text>`);return c`<div class="fc-graph">
    ...
    </ha-dialog>`}_renderStats(){let t=b(this._state("sensor.pv_total")),e=vt(this),s=this._show("show_panels")&&e.length>0,o=this._state("sensor.grid_power"),i=b(o);return c`<div class="stats">
      <div
