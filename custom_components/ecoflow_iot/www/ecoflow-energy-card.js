...
`;function vt(r){let t=[];for(let e=1;e<=4;e++){let s=r._config.panels?.[e]||{};if(s.hidden)continue;let o=`sensor.pv${e}_power`,i=r._state(o);i&&t.push({i:e,watts:b(i),id:r._entityId(o),name:s.name||null})}return t}function Qt(r){let t=(i,n)=>S(r.hass,i,n),e=vt(r);if(!e.length)return l`<div class="empty">${t("panels.none")}</div>`;let s=Math.max(1,...e.map(i=>i.watts||0)),o=e.reduce((i,n)=>i+(n.watts||0),0);return l`<div class="panels">
    ${e.map(i=>l`<div
      ...
      >
        <div class="panel-head">
          ...
          <span class="panel-val">${A(i.watts)??"\u2013"}</span>
        </div>
        <div class="pbar">
          <div
            class="pfill"
  ...
  </div>`}var yt=1e3,bt=340,v={l:46,r:14,t:16,b:28};function te(r,{actual:t,forecast:e,totalWh:s,showForecast:o,title:i}){let n=(p,m)=>S(r.hass,p,m),c=yt-v.l-v.r,a=bt-v.t-v.b,d=p=>v.l+p/24*c,u=0;for(let p=0;p<24;p++)u=Math.max(u,(t[p]||0)/1e3),o&&(u=Math.max(u,(e[p]||0)/1e3));let h=ke(u>0?u:.1),_=p=>v.t+a-p/h*a,y=c/24*.66,E=[];for(let p=0;p<24;p++){let m=(t[p]||0)/1e3;if(m<=0)continue;let D=d(p+.5)-y/2,At=_(m);E.push(z`<rect class="fc-actual" x=${D.toFixed(1)} y=${At.toFixed(1)}
        width=${y.toFixed(1)} height=${(v.t+a-At).toFixed(1)} rx="2"></rect>`)}let $t=null;if(o){let p=[];for(let m=0;m<=24;m++)p.push(`${d(m).toFixed(1)},${_((e[m]||0)/1e3).toFixed(1)}`);$t=z`<polyline class="fc-line" points=${p.join(" ")}></polyline>`}let et=[],xt=4;for(let p=0;p<=xt;p++){let m=h/xt*p,D=_(m);et.push(z`<line class="fc-grid" x1=${v.l} y1=${D.toFixed(1)} x2=${yt-v.r} y2=${D.toFixed(1)}></line>`),et.push(z`<text class="fc-axis fc-axis-y" x=${v.l-6} y=${(D+4).toFixed(1)}>${Se(m)}</text>`)}let wt=[];for(let p=0;p<=24;p+=4)wt.push(z`<text class="fc-axis fc-axis-x" x=${d(p).toFixed(1)} y=${bt-8}>${p}:00</text>`);return l`<div class="fc-graph">
    ...
    >
      ...
      >
        ...
        ${o&&s!=null?l`<span class="chip charge"
              ><ha-icon icon="mdi:flash"></ha-icon>${A(Math.abs(s))}</span
            >`:i&&s!=null?l`<span class="chip discharge"
                ><ha-icon icon="mdi:battery-arrow-down"></ha-icon>${A(Math.abs(s))}</span
              >`:""}
      </div>
      ${n&&(d!=null||c!=null||a!=null)?l`<div class="batt-flags">
            ${d!=null?this._flag("reserve","mdi:shield-home",d,this._t("card.reserve"),"number.backup_reserve"):""}
            ${c!=null?this._flag("charge","mdi:arrow-up-bold",c,this._t("card.charge_limit"),"number.max_charge_soc"):""}
            ${a!=null?this._flag("discharge","mdi:arrow-down-bold",a,this._t("card.discharge_limit"),"number.min_discharge_soc"):""}
          </div>`:""}
      <div class="bar">
    ...
    </div>`}_flag(t,e,s,o,i){let n=Math.max(0,Math.min(100,s)),c=n<=12?"0":n>=88?"-100%":"-50%";return l`<span
      class="flag ${t} clickable"
      style="left:${n}%;transform:translateX(${c})"
      title="${o} ${Math.round(s)}%"
      @click=${()=>this._moreInfo(i)}
    >
      <ha-icon icon=${e}></ha-icon>${Math.round(s)}%
      ...
      ${t.map(e=>{let s=e.swState?.state==="on",o=b(e.pwState);return l`<div
        ...
        >
          ...
          <div class="ac-info">
            ...
            <span class="ac-power">
              ${e.swState&&!s?this._t("card.off"):A(o)??(e.pwState?"\u2014":"")}
            </span>
          </div>
    ...
    </div>`}_withPanel(t,e){let s={...this._config.panels||{}},o={...s[t]||{}};for(let[n,c]of Object.entries(e))c===""||c==null||c===!1?delete o[n]:o[n]=c;Object.keys(o).length?s[t]=o:delete s[t];let i={...this._config,panels:s,type:`custom:${g}`};return Object.keys(s).length||delete i.panels,i}_renderToggle(t,e,s){return l`<div class="row">
