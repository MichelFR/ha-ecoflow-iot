...
`;function Et(r){let t=[];for(let e=1;e<=4;e++){let i=r._config.panels?.[e]||{};if(i.hidden)continue;let s=`sensor.pv${e}_power`,o=r._state(s);o&&t.push({i:e,watts:v(o),id:r._entityId(s),name:i.name||null})}return t}function ne(r){let t=(o,a)=>z(r.hass,o,a),e=Et(r);if(!e.length)return n`<div class="empty">${t("panels.none")}</div>`;let i=Math.max(1,...e.map(o=>o.watts||0)),s=e.reduce((o,a)=>o+(a.watts||0),0);return n`<div class="panels">
    ${e.map(o=>n`<div
  ...
  </div>`}var Ct=1e3,Pt=340,x={l:46,r:14,t:16,b:28};function ce(r,{actual:t,forecast:e,totalWh:i,showForecast:s,title:o}){let a=(u,y)=>z(r.hass,u,y),c=Ct-x.l-x.r,l=Pt-x.t-x.b,p=u=>x.l+u/24*c,h=0;for(let u=0;u<24;u++)h=Math.max(h,(t[u]||0)/1e3),s&&(h=Math.max(h,(e[u]||0)/1e3));let d=De(h>0?h:.1),f=u=>x.t+l-u/d*l,m=c/24*.66,b=[];for(let u=0;u<24;u++){let y=(t[u]||0)/1e3;if(y<=0)continue;let L=p(u+.5)-m/2,zt=f(y);b.push(A`<rect class="fc-actual" x=${L.toFixed(1)} y=${zt.toFixed(1)}
        width=${m.toFixed(1)} height=${(x.t+l-zt).toFixed(1)} rx="2"></rect>`)}let X=null;if(s){let u=[];for(let y=0;y<=24;y++)u.push(`${p(y).toFixed(1)},${f((e[y]||0)/1e3).toFixed(1)}`);X=A`<polyline class="fc-line" points=${u.join(" ")}></polyline>`}let $=[],E=4;for(let u=0;u<=E;u++){let y=d/E*u,L=f(y);$.push(A`<line class="fc-grid" x1=${x.l} y1=${L.toFixed(1)} x2=${Ct-x.r} y2=${L.toFixed(1)}></line>`),$.push(A`<text class="fc-axis fc-axis-y" x=${x.l-6} y=${(L+4).toFixed(1)}>${Ne(y)}</text>`)}let M=[];for(let u=0;u<=24;u+=4)M.push(A`<text class="fc-axis fc-axis-x" x=${p(u).toFixed(1)} y=${Pt-8}>${u}:00</text>`);return n`<div class="fc-graph">
    ...
    <svg class="chart" viewBox="0 0 ${Ct} ${Pt}" preserveAspectRatio="xMidYMid meet" role="img">
      ${$}${M}
  ...
  </div>`}function De(r){let t=Math.pow(10,Math.floor(Math.log10(r))),e=r/t;return(e<=1?1:e<=2?2:e<=5?5:10)*t}function Ne(r){return r>=10?Math.round(r).toString():r.toFixed(1).replace(/\.0$/,"")}var Oe=300*1e3,nt=class extends w{static styles=ae;static get properties(){return{hass:{},_config:{},_dialog:{state:!0},_confirmAc:{state:!0}}}constructor(){super(),this._dialog=null,this._confirmAc=null,this._battDir="",this._todayWh=void 0,this._hourly={},this._forecasts={},this._forecastsFetched=!1,this._period=null,this._collUnsub=null,this._collProp=void 0}connectedCallback(){super.connectedCallback(),at(),this._statsTimer=setInterval(()=>{this._refreshData(),this._refreshForecast()},Oe)}disconnectedCallback(){super.disconnectedCallback(),clearInterval(this._statsTimer),this._collUnsub&&this._collUnsub(),this._collUnsub=null,this._collProp=void 0;for(let t of Object.values(this._tmplUnsub||{}))typeof t=="function"&&t();this._tmplUnsub={}}setConfig(t){this._config=t||{}}static getConfigElement(){return document.createElement(`${_}-editor`)}static getStubConfig(){return{}}getCardSize(){return 6}_t(t,e){return z(this.hass,t,e)}_show(t,e=!0){return this._config[t]??e}get _device(){let t=ot(this.hass);return t.length?this._config.device&&t.find(e=>e.deviceId===this._config.device)||t[0]:null}_state(t){let e=this._config.entities?.[t];if(e){if(k(e)){let s=this._templateResults?.[e];return{state:s===void 0?"unknown":String(s),attributes:{}}}return P(e)?this.hass.states[e]:{state:e,attributes:{}}}let i=this._map?.[t];return i?this.hass.states[i]:void 0}_entityId(t){let e=this._config.entities?.[t];return e&&P(e)&&!k(e)?e:this._map?.[t]}updated(t){super.updated(t),(t.has("hass")||t.has("_config"))&&(this._syncTemplates(),this._bindEnergyCollection(),this._todayWh===void 0&&this._refreshData(),this._forecastsFetched||(this._forecastsFetched=!0,this._refreshForecast()))}async _refreshData(){let t=this._entityId("sensor.solar_energy");if(!t||!this.hass)return;let{start:e,end:i}=this._dataRange(),s=await Qt(this.hass,t,e,i);this._hourly=s||{},this._todayWh=s?Object.values(s).reduce((o,a)=>o+(a||0),0):null,this.requestUpdate()}async _refreshForecast(){this.hass&&(this._forecasts=await wt(this.hass),this.requestUpdate())}_dataRange(){if(this._period?.start instanceof Date)return{start:this._period.start,end:this._period.end instanceof Date?this._period.end:new Date,ref:this._period.start};let t=new Date;return{start:new Date(t.getFullYear(),t.getMonth(),t.getDate()),end:t,ref:t}}_bindEnergyCollection(){let t=this._config.collection_key,e=t?`_${t}`:null;if(e!==this._collProp&&(this._collUnsub&&this._collUnsub(),this._collUnsub=null,this._collProp=e,this._period=null),!e||this._collUnsub||!this.hass?.connection)return;let i=this.hass.connection[e];if(!i||typeof i.subscribe!="function")return;let s=()=>{this._period={start:i.start,end:i.end},this._refreshData()};this._collUnsub=i.subscribe(()=>s()),s()}_mergedForecast(){return Xt(this._forecasts,this._config.forecast_config_entries)}_forecastTodayKWh(){let t=Jt(this._mergedForecast(),this._dataRange().ref);return t!=null?t/1e3:null}_isToday(){let t=this._dataRange().ref,e=new Date;return t.getFullYear()===e.getFullYear()&&t.getMonth()===e.getMonth()&&t.getDate()===e.getDate()}_periodLabel(){return this._isToday()?this._t("card.today"):this._dataRange().ref.toLocaleDateString(this.hass?.locale?.language||void 0,{weekday:"short",day:"numeric",month:"short"})}async _syncTemplates(){if(!this.hass?.connection)return;this._tmplUnsub=this._tmplUnsub||{},this._templateResults=this._templateResults||{};let t=[...Object.values(this._config.entities||{}),this._config.title].filter(e=>k(e));for(let e of t)if(!this._tmplUnsub[e]){this._tmplUnsub[e]=!0;try{this._tmplUnsub[e]=await this.hass.connection.subscribeMessage(i=>{this._templateResults[e]=i.result,this.requestUpdate()},{type:"render_template",template:e})}catch{this._templateResults[e]="error",this.requestUpdate()}}for(let e of Object.keys(this._tmplUnsub))if(!t.includes(e)){let i=this._tmplUnsub[e];typeof i=="function"&&i(),delete this._tmplUnsub[e],delete this._templateResults[e]}}_moreInfo(t){this._moreInfoId(this._entityId(t))}_moreInfoId(t){t&&this.dispatchEvent(new CustomEvent("hass-more-info",{detail:{entityId:t},bubbles:!0,composed:!0}))}render(){if(!this.hass)return n``;let t=this._device;return t?(this._map=rt(this.hass,t.ents),this._isToday()?n`<ha-card>
      ${this._renderHead(t)}
      ${this._renderStats()}
    ...
    </ha-adaptive-dialog>`}_imageSrc(t){let e=t?.device?.model;return this._config.image_url||(this._config.image?H(this._config.image):qt(e))}_renderHead(t){let e=this._config.title,i=e&&k(e)?String(this._templateResults?.[e]??""):e,s=t.device?.model,o=i||t.device?.name_by_user||t.device?.name||s||this._t("card.device"),a=this._imageSrc(t);return n`<div class="head">
      <div class="head-left">
    ...
    </div>`}_renderBatteryCircle(t,e){let i=this._state("sensor.cms_batt_soc"),s=this._show("show_image")&&t;if(!i&&!s)return"";let o=v(i),a=v(this._state("sensor.bat_power")),c=this._state("binary_sensor.battery_charging")?.state==="on"||a!=null&&a>1,l=!c&&a!=null&&a<-1,p=c?"charge":l?"discharge":o!=null&&o<=15?"low":"",h=c?"charge":l?"discharge":"";h&&(this._battDir=h);let d=h||this._battDir,f=!!h,m=2*Math.PI*44,b=o!=null?Math.max(0,Math.min(100,o)):0,X=[{v:v(this._state("number.min_discharge_soc")),cls:"discharge",label:this._t("card.discharge_limit")},{v:v(this._state("number.backup_reserve")),cls:"reserve",label:this._t("card.reserve")},{v:v(this._state("number.max_charge_soc")),cls:"charge",label:this._t("card.charge_limit")}].filter($=>$.v!=null);return n`<div
    ...
    >
      ${i?n`<svg class="batt-ring" viewBox="0 0 100 100">
            ...
            <circle
              ...
              style="stroke-dasharray:${m.toFixed(1)};stroke-dashoffset:${(m*(1-b/100)).toFixed(1)}"
            ></circle>
            ${d?A`<circle
                  class="ring-spin ${d} ${f?"show":""}"
                  cx="50"
                  cy="50"
                  r="44"
                  stroke-dasharray="16 261"
                ></circle>`:""}
            ${X.map($=>{let E=Math.max(0,Math.min(100,$.v))/100*2*Math.PI,M=Math.sin(E),u=Math.cos(E);return A`<line
                class="ring-tick ${$.cls}"
                x1=${(50+39.5*M).toFixed(1)}
                y1=${(50-39.5*u).toFixed(1)}
                x2=${(50+48.5*M).toFixed(1)}
                y2=${(50-48.5*u).toFixed(1)}
              ><title>${$.label} ${Math.round($.v)}%</title></line>`})}
          </svg>`:""}
      ${i&&d?n`<div class="batt-particles ${d} ${f?"show":""}">
            ${Array.from({length:12},($,E)=>{let M=E*30,u=E*5%12/12*1.6;return n`<span
                class="particle"
                style="--angle:${M}deg;animation-delay:${u.toFixed(2)}s"
              ></span>`})}
          </div>`:""}
      ${s?n`<picture
            >${O(t)?n`<source
      ...
      ${f.length?n`<div class="batt-grid">
            ${f.map(m=>{let b=this._entityId(m.slot);return n`<div
                class="batt-tile ${b?"clickable":""}"
                @click=${b?()=>this._moreInfoId(b):null}
              >
    ...
    >`}_renderStats(){if(Array.isArray(this._config.stats))return n`<div class="stats custom">
      ...
      </div>`;let t=v(this._state("sensor.pv_total")),e=Et(this),i=this._show("show_panels")&&e.length>0,s=this._state("sensor.grid_power"),o=v(s);return n`<div class="stats">
      <div
    ...
    >
      ...
      </div>`}_setImage(t){let e={...this._config,type:`custom:${_}`};delete e.image_url,t?e.image=t:delete e.image,this._dispatch(e)}_renderAdvancedPage(){return n`<div class="hint top-hint">${this._t("editor.collection_hint")}</div>
      <ha-form
        .hass=${this.hass}
      ...
      ></ha-form>`}_setCollectionKey(t){let e={...this._config,type:`custom:${_}`};t?e.collection_key=t:delete e.collection_key,this._dispatch(e)}_renderStatsPage(){let t=this._config.stats;return Array.isArray(t)?n`<div class="hint top-hint">${this._t("editor.stats_hint")}</div>
    ...
    </div>`}_withPanel(t,e){let i={...this._config.panels||{}},s={...i[t]||{}};for(let[a,c]of Object.entries(e))c===""||c==null||c===!1?delete s[a]:s[a]=c;Object.keys(s).length?i[t]=s:delete i[t];let o={...this._config,panels:i,type:`custom:${_}`};return Object.keys(i).length||delete o.panels,o}_renderToggle(t,e,i){return n`<div class="row">
      ...
      ></ha-switch>
    </div>`}_slotMode(t,e){return this._modes[t]?this._modes[t]:e?P(e)&&!k(e)?"entity":"custom":pe.has(t)?"entity":"auto"}_renderModeChips(t,e){let i=pe.has(t)?["entity","custom"]:["auto","entity","custom"];return n`<div class="modes">
      ${i.map(s=>n`<button
          class="mode ${e===s?"on":""}"
      ...
      ${o==="auto"?n`<div class="hint">
            ...
            ></ha-form>`:n`<ha-form
              ...
              @value-changed=${c=>{c.stopPropagation(),this._setOverride(t,c.detail.value.value||"")}}
            ></ha-form>`}`}_withOverride(t,e){let i={...this._config.entities||{}};e?i[t]=e:delete i[t];let s={...this._config,entities:i,type:`custom:${_}`};return Object.keys(i).length||delete s.entities,s}_setOverride(t,e){this._dispatch(this._withOverride(t,e))}_toggleDisplay(t,e,i){let s={...this._config,type:`custom:${_}`};i===e?delete s[t]:s[t]=i,this._dispatch(s)}_valueChanged(t){t.stopPropagation();let e={...this._config,...t.detail.value,type:`custom:${_}`};e.device||delete e.device,this._dispatch(e)}_dispatch(t){this._config=t,this.dispatchEvent(new CustomEvent("config-changed",{detail:{config:t},bubbles:!0,composed:!0}))}static get styles(){return W`
      .nav {
        display: flex;
        flex-direction: column;
        margin-top: 16px;
