...
`;function vt(r){let t=[];for(let e=1;e<=4;e++){let s=r._config.panels?.[e]||{};if(s.hidden)continue;let o=`sensor.pv${e}_power`,i=r._state(o);i&&t.push({i:e,watts:A(i),id:r._entityId(o),name:s.name||null})}return t}function Qt(r){let t=(i,n)=>S(r.hass,i,n),e=vt(r);if(!e.length)return c`<div class="empty">${t("panels.none")}</div>`;let s=Math.max(1,...e.map(i=>i.watts||0)),o=e.reduce((i,n)=>i+(n.watts||0),0);return c`<div class="panels">
    ${e.map(i=>c`<div
  ...
  </div>`}var yt=1e3,bt=340,v={l:46,r:14,t:16,b:28};function te(r,{actual:t,forecast:e,totalWh:s,showForecast:o,title:i}){let n=(u,m)=>S(r.hass,u,m),l=yt-v.l-v.r,a=bt-v.t-v.b,h=u=>v.l+u/24*l,p=0;for(let u=0;u<24;u++)p=Math.max(p,(t[u]||0)/1e3),o&&(p=Math.max(p,(e[u]||0)/1e3));let d=ke(p>0?p:.1),f=u=>v.t+a-u/d*a,y=l/24*.66,E=[];for(let u=0;u<24;u++){let m=(t[u]||0)/1e3;if(m<=0)continue;let N=h(u+.5)-y/2,At=f(m);E.push(D`<rect class="fc-actual" x=${N.toFixed(1)} y=${At.toFixed(1)}
        width=${y.toFixed(1)} height=${(v.t+a-At).toFixed(1)} rx="2"></rect>`)}let $t=null;if(o){let u=[];for(let m=0;m<=24;m++)u.push(`${h(m).toFixed(1)},${f((e[m]||0)/1e3).toFixed(1)}`);$t=D`<polyline class="fc-line" points=${u.join(" ")}></polyline>`}let et=[],xt=4;for(let u=0;u<=xt;u++){let m=d/xt*u,N=f(m);et.push(D`<line class="fc-grid" x1=${v.l} y1=${N.toFixed(1)} x2=${yt-v.r} y2=${N.toFixed(1)}></line>`),et.push(D`<text class="fc-axis fc-axis-y" x=${v.l-6} y=${(N+4).toFixed(1)}>${Se(m)}</text>`)}let wt=[];for(let u=0;u<=24;u+=4)wt.push(D`<text class="fc-axis fc-axis-x" x=${h(u).toFixed(1)} y=${bt-8}>${u}:00</text>`);return c`<div class="fc-graph">
  ...
  </div>`}function ke(r){let t=Math.pow(10,Math.floor(Math.log10(r))),e=r/t;return(e<=1?1:e<=2?2:e<=5?5:10)*t}function Se(r){return r>=10?Math.round(r).toString():r.toFixed(1).replace(/\.0$/,"")}var Ee=300*1e3,Q=class extends b{static styles=Zt;static get properties(){return{hass:{},_config:{},_dialog:{state:!0},_confirmAc:{state:!0}}}constructor(){super(),this._dialog=null,this._confirmAc=null,this._todayWh=void 0,this._hourly={},this._forecasts={},this._forecastsFetched=!1,this._period=null,this._collUnsub=null,this._collProp=void 0}connectedCallback(){super.connectedCallback(),Z(),this._statsTimer=setInterval(()=>{this._refreshData(),this._refreshForecast()},Ee)}disconnectedCallback(){super.disconnectedCallback(),clearInterval(this._statsTimer),this._collUnsub&&this._collUnsub(),this._collUnsub=null,this._collProp=void 0;for(let t of Object.values(this._tmplUnsub||{}))typeof t=="function"&&t();this._tmplUnsub={}}setConfig(t){this._config=t||{}}static getConfigElement(){return document.createElement(`${_}-editor`)}static getStubConfig(){return{}}getCardSize(){return 6}_t(t,e){return S(this.hass,t,e)}_show(t,e=!0){return this._config[t]??e}get _device(){let t=X(this.hass);return t.length?this._config.device&&t.find(e=>e.deviceId===this._config.device)||t[0]:null}_state(t){let e=this._config.entities?.[t];if(e){if(x(e)){let o=this._templateResults?.[e];return{state:o===void 0?"unknown":String(o),attributes:{}}}return O(e)?this.hass.states[e]:{state:e,attributes:{}}}let s=this._map?.[t];return s?this.hass.states[s]:void 0}_entityId(t){let e=this._config.entities?.[t];return e&&O(e)&&!x(e)?e:this._map?.[t]}updated(t){super.updated(t),(t.has("hass")||t.has("_config"))&&(this._syncTemplates(),this._bindEnergyCollection(),this._todayWh===void 0&&this._refreshData(),this._forecastsFetched||(this._forecastsFetched=!0,this._refreshForecast()))}async _refreshData(){let t=this._entityId("sensor.solar_energy");if(!t||!this.hass)return;let{start:e,end:s}=this._dataRange(),o=await qt(this.hass,t,e,s);this._hourly=o||{},this._todayWh=o?Object.values(o).reduce((i,n)=>i+(n||0),0):null,this.requestUpdate()}async _refreshForecast(){this.hass&&(this._forecasts=await gt(this.hass),this.requestUpdate())}_dataRange(){if(this._period?.start instanceof Date)return{start:this._period.start,end:this._period.end instanceof Date?this._period.end:new Date,ref:this._period.start};let t=new Date;return{start:new Date(t.getFullYear(),t.getMonth(),t.getDate()),end:t,ref:t}}_bindEnergyCollection(){let t=this._config.collection_key,e=t?`_${t}`:null;if(e!==this._collProp&&(this._collUnsub&&this._collUnsub(),this._collUnsub=null,this._collProp=e,this._period=null),!e||this._collUnsub||!this.hass?.connection)return;let s=this.hass.connection[e];if(!s||typeof s.subscribe!="function")return;let o=()=>{this._period={start:s.start,end:s.end},this._refreshData()};this._collUnsub=s.subscribe(()=>o()),o()}_mergedForecast(){return Gt(this._forecasts,this._config.forecast_config_entries)}_forecastTodayKWh(){let t=Kt(this._mergedForecast(),this._dataRange().ref);return t!=null?t/1e3:null}_periodLabel(){let t=this._dataRange().ref,e=new Date;return t.getFullYear()===e.getFullYear()&&t.getMonth()===e.getMonth()&&t.getDate()===e.getDate()?this._t("card.today"):t.toLocaleDateString(this.hass?.locale?.language||void 0,{weekday:"short",day:"numeric",month:"short"})}async _syncTemplates(){if(!this.hass?.connection)return;this._tmplUnsub=this._tmplUnsub||{},this._templateResults=this._templateResults||{};let t=[...Object.values(this._config.entities||{}),this._config.title].filter(e=>x(e));for(let e of t)if(!this._tmplUnsub[e]){this._tmplUnsub[e]=!0;try{this._tmplUnsub[e]=await this.hass.connection.subscribeMessage(s=>{this._templateResults[e]=s.result,this.requestUpdate()},{type:"render_template",template:e})}catch{this._templateResults[e]="error",this.requestUpdate()}}for(let e of Object.keys(this._tmplUnsub))if(!t.includes(e)){let s=this._tmplUnsub[e];typeof s=="function"&&s(),delete this._tmplUnsub[e],delete this._templateResults[e]}}_moreInfo(t){this._moreInfoId(this._entityId(t))}_moreInfoId(t){t&&this.dispatchEvent(new CustomEvent("hass-more-info",{detail:{entityId:t},bubbles:!0,composed:!0}))}render(){if(!this.hass)return c``;let t=this._device;return t?(this._map=J(this.hass,t.ents),c`<ha-card>
      ${this._renderHead(t)}
      ${this._renderStats()}
      ${this._show("show_today")?this._renderToday():""}
      ${this._dialog==="panels"?this._dialogFrame(this._t("panels.title"),Qt(this)):""}
      ${this._dialog==="today"?this._dialogFrame(this._periodLabel(),te(this,{title:this._periodLabel(),actual:this._hourly||{},forecast:_t(this._mergedForecast(),this._dataRange().ref),totalWh:this._todayWh,showForecast:this._show("show_forecast")&&Object.keys(this._forecasts||{}).length>0})):""}
    ...
    </ha-dialog>`}_renderHead(t){let e=this._config.title,s=e&&x(e)?String(this._templateResults?.[e]??""):e,o=t.device?.model,i=s||t.device?.name_by_user||t.device?.name||o||this._t("card.device"),n=this._config.image_url||Bt(o);return c`<div class="head">
      <div class="head-left">
    ...
    </div>`}_renderBatteryCircle(t,e){let s=this._state("sensor.cms_batt_soc"),o=this._show("show_image")&&t;if(!s&&!o)return"";let i=A(s),n=A(this._state("sensor.bat_power")),l=this._state("binary_sensor.battery_charging")?.state==="on"||n!=null&&n>1,a=!l&&n!=null&&n<-1,h=l?"charge":a?"discharge":i!=null&&i<=15?"low":"",p=l?"charge":a?"discharge":"",d=2*Math.PI*44,f=i!=null?Math.max(0,Math.min(100,i)):0;return c`<div
    ...
    >
      ...
      ${t.map(e=>{let s=e.swState?.state==="on",o=A(e.pwState);return c`<div
    ...
    </ha-dialog>`}_renderStats(){let t=A(this._state("sensor.pv_total")),e=vt(this),s=this._show("show_panels")&&e.length>0,o=this._state("sensor.grid_power"),i=A(o);return c`<div class="stats">
      <div
    ...
    >
    ...
    </div>`}_withPanel(t,e){let s={...this._config.panels||{}},o={...s[t]||{}};for(let[n,l]of Object.entries(e))l===""||l==null||l===!1?delete o[n]:o[n]=l;Object.keys(o).length?s[t]=o:delete s[t];let i={...this._config,panels:s,type:`custom:${_}`};return Object.keys(s).length||delete i.panels,i}_renderToggle(t,e,s){return c`<div class="row">
      ...
      ></ha-switch>
    </div>`}_slotMode(t,e){return this._modes[t]?this._modes[t]:e?O(e)&&!x(e)?"entity":"custom":ie.has(t)?"entity":"auto"}_renderModeChips(t,e){let s=ie.has(t)?["entity","custom"]:["auto","entity","custom"];return c`<div class="modes">
      ${s.map(o=>c`<button
          class="mode ${e===o?"on":""}"
